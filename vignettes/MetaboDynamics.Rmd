...
knitr::include_graphics("../man/figures/README-logo.png")
```
...
conditions.
- **if no metabolite abundance is available indicate with "NA" instead of 0**
...
- For clustering of metabolite abundance dynamics at least two time points are required
...
cell counts have to be estimated (i.e. via a cell counter not counting every cell) and 
...
# Analysis time
For two conditions of 100 metabolites the fitting of the dynamics model will take 
10-15 minutes. Total workflow time is approximately 15-20 minutes.
...
insights into changes of metabolite abundances over time. Untargeted metabolomics
is often used as a mean of hypothesis generation.

However, most existing tools are limited to comparing only two time points or 
experimental conditions, and they often rely on frequentist statistical methods.
Additionally, most tools are only able to analyze groups of metabolites on a
pathway level, potentially missing more general patterns of changes in the 
metabolism.

Here, we introduce MetaboDynamics, a modular workflow of probabilistic models 
for the analysis of (untargeted) longitudinal metabolomics data. This package provides a 
...
Metabolomics data is often noisy and limited to few replicates due to high costs. 
To address these challenges, we employ a Bayesian hierarchical model that balances
between under- and over-fitting. This model provides a robust method for 
...
of the model are dynamics vectors (i.e. abundances of one metabolite in one condition
over all observed time points) and the difference between these dynamics vectors
between experimental conditions.

In this vignette, I will demonstrate how to use the MetaboDynamics package to
analyze longitudinal metabolomics data, including data processing, 
model fitting, and interpretation of results. I will show how to apply 
...
replicates at four time points (1-4) across two experimental conditions (A and B).
...
please see Vignette **"MetaboDynamics_dataframes"** as guidance. 
...
# Model dynamics
To analyze the dynamics data, we use a Bayesian hierarchical models. 
These model are used to account for the complexity and variability of the data and 
to provide a flexible and robust framework for analysis. 

We provide two model options: *sd_per_time_point* and *sd_per_condition*.

## sd per time point

For the *sd_per_time_point* option we assume a normal distribution of log-transformed
metabolite abundances.  
This model requires **at least triplicates** and assumes a normal distribution 
of log-transformed and scaled metabolite
...
With: con = metabolite abundances,
...
While this option maximizes the sensitivity of the detection of differential
metabolite abundances it is not always well suited for few replicates or high
variance between replicates. 
We advice to **check the diagnostic criteria and PPC carefully** and if in doubt
switch to *sd_per_condition*.

## sd per condition

For these cases we provide the second option 
*sd_per_condition* that requires at least **duplicates** and assumes the same 
$\sigma$ for all time points per condition
and metabolite and has partial pooling of metabolite and condition specific 
$\sigma$ into metabolite specific $\lambda$.

\begin{align*}
\log(con_{m,c,t})&\sim {\sf normal}(\mu_{m,c,t},\sigma_{m,c}) \\
\mu_{m,c,t}&\sim {\sf normal}(0,2) \\
\sigma_{m,c}&\sim {\sf exponential}(\lambda_{m}) \\
\lambda_{m}&\sim {\sf exponential}(2) 
\end{align*}
...
available adapted models can be run that takes the **raw** (i.e. not normalized and not scaled)
metabolite abundances and the cell counts as input. 
Again we have the two options *sd_per_time_point* and *sd_per_condition*.

The priors in this model are not as generic as in the other model options, as the
data is not scaled. MetaboDynamics provides default priors that fit our data well, 
but priors can be adapted to your specific experiment. For details please see
Vignette **"MetaboDynamics_Guide_to_prior_setting"** as guidance. 

With cc = cell counts and con = metabolite abundance per cell we get:

- **sd_per_time_point** 

\begin{align*}
con_{m,c,t}&\sim {\sf lognormal}(\mu_{m,c,t},\sigma_{m,c,t}) \\
\mu_{m,c,t}&\sim {\sf normal}(12,5) \\
\sigma_{m,c,t}&\sim {\sf exponential}(\lambda_{m,c}) \\
\lambda_{m,c}&\sim {\sf exponential}(2) \\
cc_{c,t}&\sim {\sf poisson}(meanCounts_{c,t}) \\
meanCounts_{c,t}&\sim{\sf exponential}(1/1e6) \\
meanNatural_{m,c,t}&=exp(\mu_{m,c,t}+\sigma_{m,c,t}) \\
apc_{m,c,t}&=meanNatural_{m,c,t}/meanCounts_{c,t} \\
apcScaled_{m,c,t}&=(log(apc_{m,c,t})-mean(log(apc_{m,c,t})))/sd(log(apc_{m,c,t})) \\
\end{align*}

- **sd_per_condition**

\begin{align*}
con_{m,c,t}&\sim {\sf lognormal}(\mu_{m,c,t},\sigma_{m,c}) \\
\mu_{m,c,t}&\sim {\sf normal}(12,5) \\
\sigma_{m,c}&\sim {\sf exponential}(\lambda_{m}) \\
\lambda_{m}&\sim {\sf exponential}(2) \\
cc_{c,t}&\sim {\sf poisson}(meanCounts_{c,t}) \\
...
fit_dynamics_model(). Just in this vignette the number of chains is set to one.
To get reliable estimates at least four chains should be used. If more than
one CPU core is available chains can run in parallel (adjustable by parameter "cores")
to reduce runtime.
Here we fit the dynamics model for a small subset of the simulated metabolites. 
...
data <- fit_dynamics_model(
  model = "scaled_log",
  model_option = "sd_per_condition", # faster fitting for vignette
  data = data, scaled_measurement = "m_scaled",
  assay = "scaled_log",
  adapt_delta = 0.95, # default 0.95
  iter = 2000,
  cores = 1,
  chains = 1 # only set to 1 for vignette, use 4 chains
)
```

This returns a stanfit object. If data is a SummarizedExperiment object the fits are stored in 
...
The metabolites are ranked on the x-axis according to their difference in abundance
between the two time points. Metatabolites which abundances decreases the most
between time points are visualized on the far left, metabolites which abundances
increase the most are visualized on the far right of the plot. 
...
means that with 95% probability the true difference of metabolite abundances
lies within the range of the error bars. 
If the 95% CrI lies below zero, we can conclude that there is a decrease in 
abundance between the two subsequent time points (color coded in red). 
If the 95% CrI lies above zero, we can conclude that there
is an increase in abundance between the two subsequent time points (color coded in green). 
If the 95% CrI contains zero, we
cannot conclude that there is a difference in abundance between the two time points given
the provided data.

### Difference of dynamics between experimental conditions
...
the estimated differences between dynamics is pretty high. Higher euclidean
distances indicate big differences between dynamics, euclidean distances close to
zero small differences between dynamics. 
...
data <- cluster_dynamics(data,
  deepSplit = 2, # low clustering sensitivity: can be set between 1 (low) and 4 (high)
...
To plot phylogenetic trees of condition B run:
...
and 
**3) a [patchwork](https://cran.r-project.org/web/packages/patchwork/index.html)
...
The functional modules of the KEGG-database are organized in three hierarchies:
...
library(patchwork)
p <- cluster$patchwork$A | ora$ora_patchwork$A
p + plot_layout(ncol = 4, widths = c(2, 1, 2, 2))
```
...
bright points indicate high dynamic similarity with small spread.
...
If we compare that to the dynamics profiles and ORA analysis
...
(i.e. big points) but dissimilar (i.e. bright color) in regards to their dynamics. Here this is 
