...
The minimal requirements are triplicates of (relative) metabolite abundances at a minimum of two time points and at least one experimental condition. There is no minimum number of metabolites required.

Metabolite abundances have to be adequately normalized. For example normalization to cell counts in cell culture.
...
slower but has the advantage of taking additionally incorporating the uncertainty of cell counts
into the estimates of the Bayesian model.  

The code below shows how to fit the Bayesian model with the function
fit_dynamics_model(). This might take of the order of 10 minutes for 98 metabolites. Here we fit the dynamics model 
...
### !!!! choose different metabolites differences in metabolite abundance between time points
...
L-Alanine B (timepoint 4 timepoint1), ATP L-Alanine timepoint 3 timepoint 1
...
can be used to detect which metabolites's dynamics change the most/least between conditions

!!! insert diagram of anonymized johanns data? 
...
##### !!!! attach ORA to patchwork
data("longitudinalMetabolomics")
data("longitudinalMetabolomics_df")
samples <- c("TTP","Sarcosine","L-Ornithine","Uracil","Xanthine","ATP","dGMP","l-Tyrosine","Malic acid","sn-Glycero-3-phosphocholine")
data <- longitudinalMetabolomics[, longitudinalMetabolomics$condition %in%c("A","B") &
                                   longitudinalMetabolomics$metabolite %in% samples]
data <- fit_dynamics_model(
  data = data,
  scaled_measurement = "m_scaled", assay = "scaled_log",
  max_treedepth = 14, adapt_delta = 0.95, iter = 3000, cores = 1, chains = 1
)
data <- estimates_dynamics(
  data = data
)
data <- cluster_dynamics(data, B = 1000)
plot_cluster <- plot_cluster(data)
plot_cluster
data("metabolite_modules")
data("modules_compounds")
data <- ORA_hypergeometric(data=data,background = modules_compounds,annotations = metabolite_modules,tested_column = "middle_hierarchy")

plot_ORA <- plot_ORA(data = data,tested_column = "middle_hierarchy",patchwork = TRUE,plot_cluster = plot_cluster)

patchwork <- list()
for (i in unique(longitudinalMetabolomics_df$condition)[1:2]){
  p <- plot_cluster$patchwork[[i]]|plot_ORA$ora_patchwork[[i]]
  patchwork[[i]] <- p+plot_annotation(title = paste0("condition ",i),subtitle="plots: dendrogram with cluster probability, cluster affiliations of metabolites, ORA")
