...
The data set is represented as a [SummarizedExperiment](https://bioconductor.org/packages/release/bioc/html/SummarizedExperiment.html) object, where the abundance 
tables of each experimental condition are stored in assays (raw abundances in "concentrations", log-transformed transformations in 
...
metabolites with each other which abundances differ by magnitude we define 
dynamics as deviations at the observed time points from the 
metabolite's mean abundance. For this we standardize each metabolite at each
...
# Model dynamics
To analyze the metabolomics data, we use a Bayesian hierarchical model. 
...
but the spread of different time points informs each other through the hierarchical
...
model fit usin the estimates_dynamics() function:
...
This function returns two types of estimates: the estimated abundance 
differences between two subsequent time points for each metabolite at each 
experimental condition, and the dynamics profiles of each metabolite at each 
...
of the parameter (i.e. differences in abundances between time points). That
...
metabolite specific dynamics vectors (estimates[,c("mu1.mean":"mut.mean)]) to 
...
criteria such as Gap statistics and average silhouette. Some clustering methods
...
The code below shows a very fundamental example of hierarchical clustering. 
...
The clustering result for the whole simulated data set is stored in the metadata
of the SummarizedExperiment object and can be accessed using the following code:
...
functional modules, were prepared with MetaboDynamics function "get_ORA_annotations" (type "?get_ORA_annotations" in R to get to the help page).
...
which dynamics cluster! For instance in cluster 8 of condition B module
...
(i.e. big points)
but dissimilar (i.e. bright color) in regards to their dynamics. In this example
this is the cluster pair B_7 and A_4. Their ORA profiles are also quite similar 
...
can be spotted by big (i.e. high metabolite similaritey) black (i.e. low dissimilarity
in dynamics) points, for example cluster pair C_6 and A_5
