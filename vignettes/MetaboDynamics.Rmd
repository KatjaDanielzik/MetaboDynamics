...
plot_diagnostics(diagnostics = diagnostics[["model_diagnostics"]],
                 data = longitudinalMetabolomics)

# PPCs can be accessed with
plot_PPC(posterior = diagnostics[["PPC_A"]], data = longitudinalMetabolomics,
         scaled_measurement = "m_scaled")
```
...
heatmap_dynamics(estimates=comparison_dynamics[["estimates"]],clusters = cluster)
```
...
0 indicate low similarity, values near 1 high similarity.
```{r,fig.aling='center',fig.dpi=150}
comparison_metabolites <- compare_metabolites(clusters = cluster)
heatmap_metabolites(distances = comparison_metabolites[["Jaccard"]],
                    clusters=cluster)
```
