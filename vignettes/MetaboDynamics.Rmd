...
png(file = "dynamics_one_metabolite.png", pointsize = 60, width = 20, height = 10, res = 1000, units = "cm")
ggplot(temp_d[temp_d$metabolite == "L-Arginine" & temp_d$condition == "2Gy", ], aes(
  x = as.factor(as.numeric(as.factor(timepoints))),
  y = mean, group = metabolite.ID
)) +
  ...
  theme(legend.position = "none") +
  facet_grid(rows = vars(condition)) +
  ggtitle("dynamic profile of L-Arginine")
dev.off()
