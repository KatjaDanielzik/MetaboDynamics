...
samples <- sample(unique(data_sim$metabolite),10)
data_sim <- as.data.frame(SummarizedExperiment::colData(data_sim))
data_sim <- data_sim[data_sim$condition=="A",]
data_sim <- data_sim[data_sim$metabolite%in%samples,]
...
on the full simulated data set (data("data_sim")). In a real life example the 
...
dataset data("data_sim"), with columns "metabolite", "condition", "mu1-t.mean"
...
```{r}
# ORA_dataframes <- get_ORA_annotations(data = data_sim, kegg = "KEGG",
