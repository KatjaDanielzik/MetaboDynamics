---
title: "MetaboDynamics_a_worked_example"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{MetaboDynamics_a_worked_example}
...
ggplot(intra) +
  geom_line(aes(x = as.numeric(as.factor(time)),
...
```{r,fig.align='center',fig.dpi=150}
# fit model
# fits_dynamics <- fit_dynamics_model(data = intra, cpc = "log_cpc_stand",
#                                     condition = "dose", max_treedepth = 14,
#                                     adapt_delta = 0.999, iter = 4000, cores = 7)

load("/media/home/fits_dynamics.RData")
# extract diagnostics
diagnostics_dynamics <- extract_diagnostics_dynamics(data=intra,iter=4000,
                                                     fits=fits_dynamics)
...
save(fits_dynamics,file="/media/home/fits_dynamics.RData")
```
...
```{r,fig.align='center',fig.dpi=150}
# extract estimates
estimates_dynamics <- extract_estimates_dynamics(data=intra,fits=fits_dynamics,
                                                 iter=4000)
```
...
dd_0Gy <- dist(estimates_dynamics[["0Gy"]][,c("mu1_mean","mu2_mean",
                                                "mu3_mean","mu4_mean")],
                 method="euclidean")
...
clust_0Gy <- estimates_dynamics[["0Gy"]][,c("metabolite","condition","mu1_mean","mu2_mean",
                                                "mu3_mean","mu4_mean")]
...
# get distances between vectors
dd_2Gy <- dist(estimates_dynamics[["2Gy"]][,c("mu1_mean","mu2_mean",
                                                "mu3_mean","mu4_mean")],
                 method="euclidean")
# hierachical clustering
...
# assing cluster ID to estimates
clust_2Gy <- estimates_dynamics[["2Gy"]][,c("metabolite","condition","mu1_mean","mu2_mean",
                                                "mu3_mean","mu4_mean")]
...
# get distances between vectors
dd_10Gy <- dist(estimates_dynamics[["10Gy"]][,c("mu1_mean","mu2_mean",
                                                "mu3_mean","mu4_mean")],
                 method="euclidean")
# hierachical clustering
...
clust_cut <- cutree(clust,k=8)
# assing cluster ID to estimates
clust_10Gy <- estimates_dynamics[["10Gy"]][,c("metabolite","condition","mu1_mean","mu2_mean",
                                                "mu3_mean","mu4_mean")]
...
```{r,fig.align='center',fig.dpi=150}
temp <- cluster
temp <- temp%>%pivot_longer(cols=c(mu1_mean,mu2_mean,mu3_mean,mu4_mean),
...
# Over-representation analysis of functional modules
