...
#'
#' @param clusters a dataframe containing the dynamics and cluster IDs(column named "cluster") of clusters of similar dynamics, as well as a column "condition" specifying the experimental condtions to be compared.
#' @param dynamics vector specifying the column names of dataframe clusters that hold the dynamic information
#'
...
#' @import ggplot2
#'
#' @return a list holding a 1) dataframe of estimates of the mean distance between
...
compare_conditions <- function(clusters,dynamics){
  ...
  for (i in 1:nrow(x)){
    for (j in 1:nrow(x)){
      ...
      if (j>i){
        ...
        a <- unlist(stringr::str_split(a,"_"))
        b <-  paste0(x[j,1],"_",x[j,2])
        b <- unlist(stringr::str_split(b,"_"))
        ...
        distances[[paste0(paste0(x[i,1],"_",x[i,2]),"vs",paste0(x[j,1],"_",x[j,2]))]] <- distance$euclidean
      }
    }
  }
  ...
  fit <- rstan::sampling(object = stanmodels$m_cluster_distance,
                         data = list(C=length(names(distances)),
                                     N=N,
                                     M=max(N),
                                     y=y_padded),
                         chains = 4,
  ...
  posterior$cluster_a <- do.call(rbind, strsplit(x = posterior$comparison, split = "vs"))[,1]
  posterior$cluster_b <- do.call(rbind, strsplit(x = posterior$comparison, split = "vs"))[,2]
  ...
  ggplot(posterior[posterior$parameter=="mu",],aes(x=cluster_b,y=cluster_a))+
    ...
    ggtitle("similarity of dynamics in clusters","estimated mean pairwise distance")
  rm(x)
  return(comparison)
}
