...
get_ORA_dataframes <- function(data, kegg = "KEGG",
   ...
   temp2 <- as.data.frame(cbind(module_id=NA, upper_hierarchy=NA, middle_hierarchy=NA,
                               lower_hierarchy=NA))
   ...
   for (i in unique(na.omit(metabolite_modules$module_id))){
     ...
     temp <- as.data.frame(cbind(module_id=i,upper_hierarchy=str_squish(temp[[1]][1]),
